...
public class Main {
     ...
     * опытом в автоматизации, а именно - навыками работы с библиотекой Rest-Assured. --что эта библиотека делает, зачем она мне нужна, если есть URL? 
    ...
    public void printThePageCode() {
        RestAssured.given()//Сделаем запрос...
                ...
                .then() // А после запроса (данный метод можно не писать, это "синтаксический сахар") --а зачем он тогда нужен?
    ...
    /**
     * Теперь в консоли много непонятной информации. Так и должно быть, значит мы движемся правильно! --что это за информация? Наверное это HTML код страницы?
     * <p>
    ...
    public void createUser() {
        RestAssured.given()
                ...
                .post(petstoreUrl + "v2/user")//Слегка видоизменю URL запроса --зачем?
                .then()
                .extract()
                .response()
                .prettyPrint();
    ...
    /** --можно просто сказать, что тело запроса отформатировано для читабельнсти. Заострять на этом внимание нет смысла, в 99% случаев это должно быть из типа содержимого
    ...
    /** --можнообъединить всё в один пример до удаления
    ...
    /**
     * Очень частно для входа на сайт необходима авторизация. --практически всегда)
     * Она бывает базовая, с помощью токена... //todo - add --OAuth etc...
    ...
    public void checkStatusCode() {
        RestAssured.given()
                ...
                .statusCode(200);//статус код - 200 --можно было вынести вначало
    }
    ...
    public void findSomethingInBigBody() {
        String name = RestAssured.given()
                ...
                .jsonPath()//и найдём в этом Json'е по пути --по выражению звучит лучше по русски
                .getString("tags.find{tag -> tag.id == 84}.name");//строку --а если надо число? 
        ...
        // найди "tags", у этого массива найди первый объект, у которого "id" равен 548, у него возьми "name" --очепятка 548 или 48?)
    }
    ...
    /**
     * А можно вообще вынести повторяющиеся строки в отдельный метод: --А лучше вынести повторяющиеся строки в отдельный метод
     */
    ...
    /**
     * И методы ещё сильнее сократятся! --тесты станут более выразительны
     */
    @Test
